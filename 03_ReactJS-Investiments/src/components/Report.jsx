export default function Report({children: report}) {
    ...
    const bgColor = (percent > 0 ? "text-green-400" : percent < 0 ? "text-red-400" : "");
    ...
    return (
        <div className="flex flex-row border-b border-gray-300 items-center">
            ...
            <p className={bgColor}>R$ {value}</p>
            <p className={"ml-auto text-sm font-semibold "+bgColor}>{strPercent}%</p>
        </div>
    )
}
